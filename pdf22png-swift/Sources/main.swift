...
struct PDF22PNGCommand: ParsableCommand {
            ...
            if parts.count == 2 {
                ...
                if widthStr.isEmpty && !heightStr.isEmpty {
                    ...
                    if let height = Int(heightStr) {
                        return .dimensions(width: 0, height: height)
                    }
                } else if !widthStr.isEmpty && heightStr.isEmpty {
                    // Wx format
                    if let width = Int(widthStr) {
                        return .dimensions(width: width, height: 0)
                    }
                } else if let width = Int(widthStr), let height = Int(heightStr) {
                    // WxH format
    ...
    private func calculateScaleFactor(spec: ScaleSpecification, pageRect: CGRect) -> CGFloat {
        ...
        case .dimensions(let width, let height):
            if width == 0 && height > 0 {
                ...
                return CGFloat(height) / pageRect.height
            } else if width > 0 && height == 0 {
                // Fixed width, auto height
                return CGFloat(width) / pageRect.width
            } else {
    ...
    private func generateOutputPath(baseName: String, pageNumber: Int) -> String {
        ...
        if all || directory != nil {
            let outputDir = directory ?? dir
            return "\(outputDir)/\(nameWithoutExtension)-\(String(format: "%03d", pageNumber)).png"
        } else {
            return "\(dir)/\(nameWithoutExtension).png"
        }
    }
}